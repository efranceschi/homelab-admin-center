#!/usr/bin/env bash
# ============================================================================
# Homelab Admin and Control Kernel (H.A.C.K.) — one-line web installer.
#
...
# the `hack` systemd service. Run as root on the Proxmox node.
#
# Environment overrides:
#   HACK_INSTALL_DIR  target directory   (default: /opt/hack)
#   HACK_BRANCH       git branch         (default: prod  — the production branch)
#   HACK_REPO         git repository URL (default: the public repo)
#   HACK_NO_START=1   install but do not start the service
# ============================================================================
set -euo pipefail

REPO_URL="${HACK_REPO:-https://github.com/efranceschi/hack.git}"
INSTALL_DIR="${HACK_INSTALL_DIR:-/opt/hack}"
BRANCH="${HACK_BRANCH:-prod}"
...
say "Installing Homelab Admin and Control Kernel -> ${INSTALL_DIR} (branch ${BRANCH})"
...
if [ -d "${INSTALL_DIR}/.git" ]; then
    ...
    die "${INSTALL_DIR} exists and is not a git checkout. Move it or set HACK_INSTALL_DIR."
else
...
say "Manage with:  systemctl status hack   |   journalctl -u hack -f"
